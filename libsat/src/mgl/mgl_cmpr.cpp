...
namespace Sat {
  ...
  int VramTableEntry::width() const {
    return ((cmdsize & 0x3F00) >> 8) * 8;
  }
  
  int VramTableEntry::height() const {
    return (cmdsize & 0x00FF);
  }
  
  void VramTableEntry::setDimensions(int w, int h) {
    // update cmdsize
    int newcmdsize = w;
    newcmdsize >>= 3;
    newcmdsize <<= 8;
  ...
  // cmdsize)
  bool VramTableEntry::isDoingSomethingStupid(int w, int h) {
//    int w = width();
//    int h = height();
...
/*    if (-(cmdxa * 2) != w) return true;
  ...
  int VramTableEntry::sourceOffset() const {
    return (cmdsrca * 8);
  }
  
  void VramTableEntry::setSourceOffset(int offset) {
    cmdsrca = (offset / 8);
  }
  
  int VramTableEntry::colorMode() const {
    return (cmdpmod & 0x0038) >> 3;
  }
  
  int VramTableEntry::paletteOffset() const {
    return cmdcolr * 8;
  }
  
  void VramTableEntry::setPaletteOffset(int offset) {
    cmdcolr = (offset / 8);
  }
  
  int VramTableEntry::fromData(const char* src) {
  ...
  int VramTableEntry::toData(char* dst) const {
  ...
  void VramTableEntry::load(std::istream& ifs) {
    ifs >> cmdpmod
        >> cmdcolr
        >> cmdsrca
        >> cmdsize
  ...
  void VramTableEntry::save(std::ostream& ofs) const {
    ofs << cmdpmod << " "
        << cmdcolr << " "
        << cmdsrca << " "
        << cmdsize << " "
  ...
  void GlueFileEntry::load(std::istream& ifs) {
    ifs >> num
        >> imageFileName
        >> paletteFileName;
  ...
  void GlueFileEntry::save(std::ostream& ofs) const {
    ofs << num << " "
        << imageFileName << " "
        << paletteFileName << " ";
